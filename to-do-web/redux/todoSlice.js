...
export const fetchCreateList = createAsyncThunk('lists/createList', async(newListTitle) => {
...
export const fetchUpdateTask = createAsyncThunk('/tasks/updateTask', async(values) => {
...
const todoSlice = createSlice({
  ...
  reducers: {
    ...
    handleCompletedOrDecompletedTask: (state,action) => {
      ...
      if (!tasks.some((currentTask) => currentTask.description === description)) {
        completedTasks.map(compTask => {
          if (compTask.description === description) {
            state.tasks.push({completed: !completed, description: description})
            state.completedTasks = completedTasks.filter((item) => 
              item.description !== description)
          }})
      }else{
        tasks.map((task) => {
          if (task.description === description) {
            state.completedTasks.push({completed: !completed, description: description})
  ...
  extraReducers: builder => {
    ...
    .addCase(fetchUpdateTask.fulfilled, (state, action) => {
      ...
      const oldDescIndex = tasks.findIndex(elem => elem.description === description)
      if (oldDescIndex => 0) {
        tasks[oldDescIndex] = {completed: completed, description: newDescription}
        state.tasks = tasks
      }
    })
  }
...
export const { selectAList, createNewTask, handleCompletedOrDecompletedTask, setErrorState} = todoSlice.actions;
